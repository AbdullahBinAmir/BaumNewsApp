...
const GetArticleContent = ({id,pname,url,navigation}) => {
  ...
  const handlePanResponderMove = (event, gestureState) => {
    if (Math.abs(gestureState.dx) > Math.abs(gestureState.dy)) {
      // User is swiping horizontally, enable WebView scrolling
      setScrollEnabled(false);
    } else {
      // User is swiping vertically, disable WebView scrolling
  ...
  const onWebViewMessage = (event) => {
    ...
    const { url, cookies } = JSON.parse(data);
    
    // Store the cookies for the current URL
    CookieManager.setFromResponse(url, cookies)
      .then(() => {
        console.log('Cookies stored for', url);
      })
      .catch((error) => {
...
/*   const handleWebViewScroll = () => {
  ...
  useEffect(()=>{
    isScroll ? navigation.setOptions({
  ...
  return (  
        <WebView
            ...
            containerStyle={{height:'100%',width:SCREEN_WIDTH}}
